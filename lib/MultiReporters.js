...
function MultiReporters(runner, options) {
    ...
    if (_.get(options, 'execute', true)) {
        var mmrOutput = _.get(options, 'reporterOptions.mmr-output');
        ...
        this._reporters = _.get(options, 'reporterEnabled', 'tap').split(',').map(
            function processReporterEnabled(name, index) {
                ...
                if (mmrOutput && reporterOptions && reporterOptions.output) {
                    reporterOptions.output = reporterOptions.output.replace(/{num}/g, mmrOutput);
                }
                ...
                if (Reporter === null) {
                    ...
                    try {
                        ...
                        originalError = err;
                        var qualifiedFile = path.resolve(process.cwd(), name);
                        try {
