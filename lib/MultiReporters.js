...
function MultiReporters(runner, options) {
    ...
    if (_.get(options, 'execute', true)) {
        ...
        var reportersToMap;
        if (_.get(options, 'reporterEnabled', ['tap']).constructor === Array) {
          reportersToMap = _.get(options, 'reporterEnabled', ['tap'])
        }
        else {
          reportersToMap = _.get(options, 'reporterEnabled', 'tap').split(',')
        }

        this._reporters = reportersToMap.map(
            function processReporterEnabled(name, index) {
                //
                // Reporter is a function(runner, option)
                if (_.isFunction(name)) {
                  var reporterOptions = this.getReporterOptions(options, name.name)
                  return new Reporter(runner, {reporterOptions: reporterOptions})
                }
                ...
                if (Reporter !== null) {
                    ...
                    console.error('Reporter was not found!', name);
                }
            }.bind(this)
        );
    }
